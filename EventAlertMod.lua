...
--------------------------------
EventAlert_UpdateInterval 				= 0.05 --Second
--------------------------------
EventAlert_DEBUG = {}
--------------------------------
			...
			if (SpellEnable) then
				...
				if eaf ~= nil then
					...
					if not eaf:IsVisible() then
						...
						Lib_ZYF:FrameSetOnUpdateOnce(eaf, EventAlert_UpdateInterval ,EventAlert_OnSCDUpdate, spellID)
	...
	if (eaf ~= nil) then
		...
		if EA_ChargeCurrent then
			...
			if (EA_Enable == 1) then
				...
				local EA_GCD = 1.5
				if (UnitClass("player")==EA_CLASS_ROGUE or UnitClass("player")==EA_CLASS_DRUID) then
					EA_GCD = 1
				end
				 EA_GCD = EA_GCD / (1+(UnitSpellHaste("player")/100))
	...
	if (EA_Config.ShowFrame == true) then
		...
		for k,v in ipairs(EA_CurrentBuffs) do
			...
			if eaf ~= nil then
				...
				Lib_ZYF:FrameSetOnUpdateOnce(eaf,EventAlert_UpdateInterval,EventAlert_OnUpdate,spellID)
				-- eaf:SetScript("OnUpdate", function(self,elapsedTime)
	...
	if (EA_Config.ShowFrame == true) then
		...
		for k,v in ipairs(EA_TarCurrentBuffs) do
			...
			if eaf ~= nil then
				...
				eaf.spellTimer:SetFont(EA_FONTS, EA_Config.TimerFontSize, "OUTLINE")
				eaf.spellStack:SetFont(EA_FONTS, EA_Config.StackFontSize, "OUTLINE")
				Lib_ZYF:FrameSetOnUpdateOnce(eaf, EventAlert_UpdateInterval, EventAlert_OnTarUpdate, spellID)
				-- eaf:SetScript("OnUpdate", function(self,elapsedTime)
	...
	if (EA_Config.ShowFrame == true) then
		...
		for i = 1, MAX_RUNES do
			...
			if (iRuneType >= 1) and (iRuneType < 4 ) then
				...
				eaf[i]:SetPoint(EA_Position.Anchor, prevFrame, EA_Position.Anchor, (IconSize+(i-2) * -xOffset*0.6), 3*(IconSize+(i-2) * yOffset*0.6))
		...
		if (EA_Unit ~= "") then
			if (EA_Config.ShowFrame == true) then
				...
				if (fNewToShow) then
					...
					Lib_ZYF:FrameSetOnUpdateOnce(eaf, EventAlert_UpdateInterval,EventAlert_OnLifeBloomUpdate)
					-- eaf:SetScript("OnUpdate", function(self,elapsedTime)
