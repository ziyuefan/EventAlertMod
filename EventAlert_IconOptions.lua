...
function EventAlert_Icon_Options_Frame_AdjustTimerFontSize()
	...
	if (EA_Config.ChangeTimer == true) then	--若計時顯示在框架內
		...
		if (EA_Config.UseFloatSec > 0) then
			EA_Config.TimerFontSize = (EA_Config.IconSize ) * 0.50		--框架內倒數大小比例(有小數點)
		else
			EA_Config.TimerFontSize = (EA_Config.IconSize ) * 0.45		--框架內倒數大小比例(無小數點)
		end
		EA_Config.StackFontSize = (EA_Config.IconSize ) * 0.35			--堆疊計數大小比例
	else	--若計時顯示在框架外
